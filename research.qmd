...
<!--
  ══════════════════════════════════════════════════
  DARK MODE — single toggle injected into navbar
  ══════════════════════════════════════════════════
-->
<script>
(function(){
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  btn.innerHTML =
    '<svg class="dm-moon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>' +
    '<svg class="dm-sun" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>';
...
# Clean column names
raw_names <- names(tbl)
clean     <- tolower(trimws(raw_names))
...
# The row number column is the unnamed first column (shows as X__ or similar)
# Use the actual numeric index from the xlsx "_" column if available,
# otherwise use row index
if ("num" %in% names(tbl)) {
  # already mapped
} else {
  tbl$num <- seq_len(nrow(tbl))
}

# Replace NA / literal "NA"
tbl[is.na(tbl)] <- ""

# Keep only our known columns in a clean data frame
out <- data.frame(
...
function osBadges(d){
  ...
  if(isValid(d.materials))
    b += '<a class="os-badge-link" href="'+d.materials.trim()+'" target="_blank" title="Open Materials / Data">'
      +'<img class="os-badge-img" src="'+BADGE.openmaterials+'" alt="Open Materials"></a>';
  /* First URL only if multiple prereg links */
  if(isValid(d.prereg)){
    var preregUrl = d.prereg.trim().split(/\s+/)[0];
    b += '<a class="os-badge-link" href="'+preregUrl+'" target="_blank" title="Preregistered">'
      +'<img class="os-badge-img" src="'+BADGE.preregistered+'" alt="Preregistered"></a>';
  }
  if(isValid(d.preprint))
    b += '<a class="os-badge-link" href="'+d.preprint.trim()+'" target="_blank" title="Preprint available">'
      +'<img class="os-badge-img" src="'+BADGE.opendata+'" alt="Preprint"></a>';
...
function buildRow(d){
  ...
  var topicBadges = topics.map(function(t){
    return '<span class="rt-topic">'+t+'</span>';
  }).join(' ');

  var num = d.num || d['_'] || '';
  var expId = 'rt-exp-'+num+'-'+Math.random().toString(36).slice(2,6);
  var searchStr = (String(d.ref||'')+' '+String(d.topic||'')).toLowerCase();

  /* Expanded detail links */
  var links = '';
  if(isValid(d.materials))
    links += '<div><a href="'+d.materials.trim()+'" target="_blank" style="color:var(--accent)">Materials &amp; Data</a></div>';
  if(isValid(d.prereg)){
    /* handle multi-line prereg urls */
  ...
  if(isValid(d.preprint))
    links += '<div><a href="'+d.preprint.trim()+'" target="_blank" style="color:var(--accent)">Preprint</a></div>';

  var studentBadge = (d.student && String(d.student).toLowerCase().includes('yes'))
    ? '<div><span style="background:var(--accent-dim);color:var(--accent);border-radius:5px;padding:2px 8px;font-size:0.71rem;font-weight:600;font-family:\'DM Sans\',sans-serif">Student collaboration</span></div>'
    : '';

  return '<tr class="rt-main-row" data-topic="'+String(d.topic||'')+'" '
    +'data-search="'+searchStr+'" '
    +'onclick="rtToggle(\''+expId+'\',this)" title="Click for details">'
    +'<td><strong style="color:var(--accent);font-family:\'DM Sans\',sans-serif">'+num+'</strong></td>'
    +'<td>'+topicBadges+'</td>'
    +'<td style="font-size:0.85rem;font-family:\'DM Sans\',sans-serif">'+String(d.ref||'')+'</td>'
    +'<td><span class="rt-oa '+oaClass(d.oatype)+'">'+oaShort(d.oatype)+'</span></td>'
    +'<td><div class="os-badges">'+osBadges(d)+'</div></td>'
    +'<td><button class="rt-expand-btn" id="btn-'+expId+'" title="Expand">'
    +'<svg width="13" height="13"><use href="#ic-chevron"/></svg></button></td>'
    +'</tr>'
    +'<tr class="rt-expanded-row hidden" id="'+expId+'">'
    +'<td colspan="6">'
    +'<div style="display:flex;gap:20px;flex-wrap:wrap;align-items:flex-start;font-family:\'DM Sans\',sans-serif">'
    +'<div style="flex:1;min-width:200px">'
    +'<strong style="color:var(--accent);font-size:0.72rem;letter-spacing:.05em;text-transform:uppercase">Reference</strong><br>'
    +'<span style="color:var(--text);font-size:0.875rem">'+String(d.ref||'')+'</span></div>'
    +'<div style="display:flex;flex-direction:column;gap:6px;font-size:0.855rem;min-width:170px">'
    +studentBadge+links
    +'</div></div></td></tr>';
}
...
function rtRender(){
  ...
  var filtered = RT_DATA.filter(function(d){
    var topicStr = String(d.topic||'').toLowerCase();
    var topicOk  = rtCurTopic === 'all' || topicStr.includes(rtCurTopic.toLowerCase());
    var searchStr = (String(d.ref||'')+' '+String(d.topic||'')).toLowerCase();
    var searchOk  = !rtCurSearch || searchStr.includes(rtCurSearch);
...
/* Run once RT_DATA is available */
if(typeof RT_DATA !== 'undefined'){
  document.addEventListener('DOMContentLoaded', rtRender);
} else {
  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(rtRender, 100);
  });
}
</script>
