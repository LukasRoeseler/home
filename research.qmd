...
# Standardise column names to known JS keys
names(tbl) <- dplyr::case_when(
  grepl("^X\\.|^\\.",          names(tbl)) ~ "num",
  grepl("Topic",               names(tbl), ignore.case=TRUE) ~ "topic",
  grepl("Reference",           names(tbl), ignore.case=TRUE) ~ "ref",
  grepl("OA",                  names(tbl), ignore.case=TRUE) ~ "oatype",
  grepl("Materials|Data",      names(tbl), ignore.case=TRUE) ~ "materials",
  grepl("Preregistration",     names(tbl), ignore.case=TRUE) ~ "prereg",
  grepl("Pre.Print|Preprint",  names(tbl), ignore.case=TRUE) ~ "preprint",
  grepl("Student",             names(tbl), ignore.case=TRUE) ~ "student",
  TRUE ~ names(tbl)
)

# Keep only the columns we need, fill missing
for (col in c("num","topic","ref","oatype","materials","prereg","preprint","student")) {
  if (!col %in% names(tbl)) tbl[[col]] <- ""
}
tbl[is.na(tbl)] <- ""

out <- tbl[, c("num","topic","ref","oatype","materials","prereg","preprint","student")]
out[] <- lapply(out, as.character)

cat('<script>\nvar RT_DATA =', jsonlite::toJSON(out, auto_unbox=TRUE), ';\n</script>')
```
